...
mod tests {
    ...
    fn test_dimension_deserialize() {
        ...
        let cfg: Config = toml::from_str(r#"dim = "auto""#).unwrap();
        assert_eq!(cfg.dim, Dimension::Auto);

        let cfg: Config = toml::from_str("dim = { min = 80, max = 120 }").unwrap();
        assert_eq!(
            cfg.dim,
            Dimension::Limited(SteppedRange::default().with_min(80).with_max(120))
        );

        let cfg: Config = toml::from_str("dim = 100").unwrap();
        assert_eq!(cfg.dim, Dimension::Fixed(100));
    }
}
